	...
	try:
		if l==0:
			...
			if m==-3:
				...
				y_a = (((x**2-y**2)*z)/r**3)*(105/(4*np.pi))**0.5
			elif m==3:
...
def radial_wave_func(n,l,r):
	if n==1:
		...
		if l==0:
			...
			r= (1/(64*5**0.5))*(a**-1.5)*(r/a)**2*(1-(1/12)*(r/a))*np.exp(-r/(4*a))
		elif l==3:
...
def hydrogen_wave_func(n,l,m,roa,nx,ny,nz):
	...
	ls_mesh = np.meshgrid(ls_y,ls_x,ls_z)
	
	y, x, z = np.vectorize(return_cartesian)(*ls_mesh)
	...
	return ls_mesh[1],ls_mesh[0],ls_mesh[2],density
